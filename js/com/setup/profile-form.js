...
class SetupProfileForm extends LitElement {
  ...
  static get styles () {
    return [inputsCSS, buttonsCSS, css`
    form {
      display: flex;
      padding: 20px 0;
    }
  ...
  async submit () {
    ...
    // TODO
    console.log('TODO save', this.values)
  ...
  render() {
    return html`
      <form>
        <div class="img-ctrl">
          <img src=${this.values.thumbDataURL || `asset:thumb:${this.userUrl}?cache_buster=${Date.now()}`}>
          <input type="file" accept=".jpg,.jpeg,.png" @change=${this.onChooseThumbFile}>
          <button @click=${this.onClickChangeThumb}>Choose Picture</button>
        </div>
        <div class="details">
          <label for="title">Name ${this.errors.title ? html`<span class="error">${this.errors.title}</span>` : ''}</label>
          
          <input autofocus name="title" tabindex="2" value=${this.values.title || ''} placeholder="Required" @change=${this.onChangeTitle} class=${this.errors.title ? 'has-error' : ''} />

          <label for="description">Bio / Description ${this.errors.description ? html`<span class="error">${this.errors.description}</span>` : ''}</label>
          <textarea name="description" tabindex="3" placeholder="Optional" @change=${this.onChangeDescription} class=${this.errors.description ? 'has-error' : ''}>${this.values.description || ''}</textarea>
        </div>
      </form>
    `
  }
  ...
  onChooseThumbFile (e) {
    ...
    fr.onload = () => {
      this.values.thumbExt = file.name.split('.').pop()
      this.values.thumbDataURL = /** @type string */(fr.result)
      this.requestUpdate()
    }
  ...
  onChangeTitle (e) {
    this.values.title = e.target.value.trim()
    this.values.label = slugify(this.values.title).toLowerCase()
  }

  onChangeDescription (e) {
    this.values.description = e.target.value.trim()
  }
}

customElements.define('setup-profile-form', SetupProfileForm)
